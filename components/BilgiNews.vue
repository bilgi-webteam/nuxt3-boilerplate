<template>
  <div>
    ...
    <div v-else-if="filteredNews.length || filteredEvents.length">
      ...
      <NuxtLink v-for="item in filteredNews" :to="item.url" :key="item.id" class="flex flex-col gap-4 mb-2 !border-none rounded-md text-slate-800 bg-slate-200 md:flex-row last:mb-0 dark:bg-slate-700 dark:text-slate-200 hover:bg-slate-400/50 dark:hover:bg-slate-800" target="_blank">
      ...
      <NuxtLink v-for="item in filteredEvents" :to="item.url" :key="item.id" class="flex flex-col gap-4 mb-2 !border-none rounded-md text-slate-800 bg-slate-300 md:flex-row last:mb-0 dark:bg-slate-800 dark:text-slate-200 hover:bg-slate-500/50 dark:hover:bg-slate-900" target="_blank">
        <div v-if="item.image" class="object-cover overflow-hidden rounded-t-lg md:rounded-l-lg md:rounded-tr-none md:max-h-28 md:max-w-80">
          <img :src="item.image" :alt="item.short_16words" class="w-auto">
        </div>
        <div class="mx-4 mt-4 md:mx-0">
